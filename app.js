...
function addFiles(newFiles) {
    newFiles.forEach(file => {
        ...
        files.push({
            id: fileId,
            file: file,
            status: 'pending', // pending, checking, locked, unlocked, error
            needsPassword: false,
            password: '',
            error: null
        });
...
function createFileItem(fileData) {
    ...
    div.innerHTML = `
        ...
        ${fileData.needsPassword && fileData.status !== 'unlocked' ? `
...
async function downloadFiles() {
    ...
    if (mergeCheckbox.checked && unlockedFiles.length > 1) {
...
async function downloadMergedPDF(unlockedFiles) {
    ...
    try {
        ...
        a.href = url;
        a.download = 'merged_unlocked.pdf';
